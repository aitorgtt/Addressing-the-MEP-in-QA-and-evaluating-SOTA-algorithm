...
token = 'introduce your token'
dw = DWaveSampler(token=token)
target_nodelist, target_edgelist, target_adjacency = dimod.child_structure_dfs(dw)

def find_embedding_plus(source_edgelist, n, p, idx):
    sys.stdout = open(f'./outputs/er/output_er_{n}_{p}_{idx}.txt', 'wt')
    try:
        out = find_embedding(source_edgelist, target_edgelist, verbose=3, interactive=True,
                             return_overlap=True)
    except:
...
for density in range(20):
    for size in range(59):
        ...
        source_edgelist = er_generator(sizes[size], densities[density], save=True, generate_weights=False)

        t0 = time.time()
        emb_list = Parallel(n_jobs=64)(
            delayed(find_embedding_plus)(source_edgelist, sizes[size], densities[density], j)
            for j in range(64))
        t1 = time.time()
        times[density, size] = t1 - t0
        
        embedding_container.append(emb_list)
...
with open('pickles/er.pkl', 'wb') as f:
